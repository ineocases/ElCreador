...
// El resto de publicaciones se simula con un volumen razonable para 3 meses, según el nicho.
...
function baseVistasPorVideo(player, calidad = 1) {
    ...
    // ESCALA DE AUDIENCIA REALISTA:
    // un video normal nace de la audiencia del canal. El descubrimiento ayuda,
    // pero ya no puede sustituir a los suscriptores ni hundir un canal grande.
    ...
    ratio *= 0.78 + clamp(constancia / 100, 0, 1) * 0.30;
    ratio *= 0.82 + clamp(algoritmo / 100, 0, 1) * 0.30;
    ratio *= 0.86 + clamp(edicion / 100, 0, 1) * 0.22;
    ratio *= 0.90 + clamp(marketing / 100, 0, 1) * 0.16;
    ratio *= 0.90 + clamp(fama / 100, 0, 1) * 0.18;

    const efecto = bonusPretemporada(player);
    if (efecto === "marketing") ratio *= 1.12;
    if (efecto === "algoritmo") ratio *= 1.10;
    if (efecto === "edicion") ratio *= 1.12;

    const tendencia = calcularTendencia();
    ratio *= tendencia * boostViews * edadFactor * clamp(Number(calidad) || 1, 0.70, 1.20);

    // Descubrimiento adicional: importante para canales pequeños, pero acotado
    // para que 100 publicaciones no terminen en decenas de miles de vistas.
    let descubrimiento;
    if (subs < 1000) descubrimiento = random(250, 900) + Math.floor(edicion * 8);
    else if (subs < 10000) descubrimiento = random(150, 600) + Math.floor(edicion * 5);
    else if (subs < 100000) descubrimiento = random(100, 500) + Math.floor(edicion * 3);
    else descubrimiento = random(100, 450) + Math.floor(edicion * 2);

    const audiencia = Math.round(subs * ratio);
    return Math.max(50, audiencia + descubrimiento);
}
...
function simularVideoSecundario() {
    ...
    let probViral =
        0.04 +
        (Number(a.creatividad) || 0) * 0.008 +
        (Number(p.fama) || 0) * 0.0018;
    if (p?.pretemporada?.efecto === "creatividad") probViral *= 1.35;
    probViral = clamp(probViral, 0.04, 1.60);

    const viral = Math.random() * 100 < probViral;
    if (viral) vistas *= randomFloat(2.0, 4.5);
...
export function procesarPublicacionTrimestre(
    ...
    // El minijuego modifica SOLO el video destacado.
    // Excelente (>=90): duplica los subs de ese video y x1.4 sus vistas.
    // Fallo (<35): penalización moderada, sin matar el trimestre.
    ...
    if (Number.isFinite(miniScore)) {
        ...
        if (original) {
            const vistasOriginales = Math.max(0, Number(original.vistas) || 0);
            const subsOriginales = Math.max(0, Number(original.suscriptores) || 0);
            const vistasAntes = Math.max(0, Number(manualResult.vistas) || 0);
            const subsAntes = Math.max(0, Number(manualResult.suscriptores) || 0);

            if (miniScore >= 90) {
                manualResult.vistas = Math.max(0, Math.round(vistasAntes * 1.40));
                manualResult.suscriptores = Math.max(0, Math.round(subsAntes * 2));
                manualResult.miniResultado = "excelente";
                manualResult.miniBonus = { vistas: 1.40, suscriptores: 2.00 };
            } else if (miniScore < 35) {
                manualResult.vistas = Math.max(0, Math.round(vistasAntes * 0.85));
                manualResult.suscriptores = Math.max(0, Math.round(subsAntes * 0.90));
                manualResult.miniResultado = "fallo";
                manualResult.miniPenalizacion = { vistas: 0.85, suscriptores: 0.90 };
                p.comunidad = Math.max(0, Number(p.comunidad || 50) - 1);
                p.reputacion = Math.max(0, Number(p.reputacion || 50) - 1);
            ...
            // procesarPublicacionVideo ya aplicó el resultado original.
            // Ajustamos solamente la diferencia producida por el minijuego.
            const deltaVistas = manualResult.vistas - vistasOriginales;
            const deltaSubs = manualResult.suscriptores - subsOriginales;
            p.vistasTotales = Math.max(0, Number(p.vistasTotales || 0) + deltaVistas);
            p.suscriptores = Math.max(0, Number(p.suscriptores || 0) + deltaSubs);
            actualizarFamaPorSubs(p);
    ...
    const efectoConstancia = gameState.player.pretemporada?.efecto === "constancia" ? 0.12 : 0;
    const niche = String(gameState.player.niche || "Gaming");
    const rangos = {
        "Gaming": [10, 32], "Fútbol": [8, 28], "Vlog": [8, 24], "Tecnología": [6, 20],
        "Cocina": [6, 20], "Periodismo": [4, 16], "Streaming": [12, 36], "IRL": [8, 26],
        "Comedia": [6, 22], "Educación": [4, 16], "Documental": [3, 12], "Podcast": [4, 14], "Fitness": [6, 20]
    };
    const [minVideos, maxVideos] = rangos[niche] || [8, 26];
    const power = Math.max(0.65, 1.05 - (constancia / 100) * 0.22 - efectoConstancia);
    const totalVideos = minVideos + Math.floor(Math.pow(Math.random(), power) * (maxVideos - minVideos + 1));
    ...
    // escala con los suscriptores actuales del canal, sus atributos y el
    // descubrimiento. El cálculo apunta a una relación visible entre subs y
    // vistas por video, en vez de comprimir el trimestre. El video destacado sigue siendo especial por su propio
