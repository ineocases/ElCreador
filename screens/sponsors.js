...
function continuar(){ setTimeout(()=>{ if(gameState.time.trimestre===1){gameState.finalizarAño();window.location.hash="#yearSummary";} else window.location.hash="#videoResult"; },180); }
export function renderSponsors(el){
 ...
 ${offer?`<section class="panel contract-card sponsor-offer-card"><div class="eyebrow">📩 PROPUESTA RECIBIDA</div><h2>${offer.name}</h2><p>${offer.name} quiere trabajar con <b>${gameState.player.canal}</b>.</p><div class="contract-stats"><div><span>Pago</span><b>$${nf(offer.pago)}</b></div><div><span>Duración</span><b>${offer.duration} año${offer.duration===1?"":"s"}</b></div><div><span>Prestigio</span><b>+${nf(offer.prestige)}</b></div></div>${offer.tipo==="casino"||offer.tipo==="cripto"?`<p class="muted">⚠️ Es una propuesta de alto riesgo reputacional.</p>`:""}<div class="contract-actions"><button id="negotiateSponsor" class="btn ghost">NEGOCIAR +20%</button><button id="acceptSponsor" class="btn primary">ACEPTAR</button><button id="rejectSponsor" class="btn ghost">RECHAZAR</button></div></section>`:`<section class="panel empty-opportunity"><div class="empty-icon">💼</div><h2>No tenés una propuesta pendiente.</h2><p>Seguí creando. Las marcas aparecen solas cuando tu canal cumple sus requisitos.</p></section>`}
