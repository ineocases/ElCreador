...
export function renderVideoResult(el) {
    ...
    const finDeAño = gameState.time.trimestre === 4;
    container.innerHTML = `
        <div class="page-shell result-page">
            ${renderHeaderHud()}

            <div class="result-kicker">
                📊 CIERRE DEL TRIMESTRE ${gameState.time.trimestre}/4
            </div>
            ...
            <div class="continue-row single-next">
                <button id="continueAfterVideo" class="btn primary big next-button">
                    ${finDeAño ? "🏆 SIGUIENTE: RESUMEN DEL AÑO" : `▶ SIGUIENTE: TRIMESTRE ${Number(gameState.time.trimestre)+1}`}
                </button>
            </div>
        </div>
    `;
