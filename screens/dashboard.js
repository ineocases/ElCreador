...
export function renderDashboard(el) {
    ...
    container.innerHTML = `
        <div class="page-shell dashboard-page">
            ...
            ${!collab && gameState.lastCollab?.estado && gameState.lastCollab?.creatorName ? `<div class="callout collab-callout dramatic-callout"><div class="callout-icon">🤝</div><div><b>${gameState.lastCollab.estado === "aceptada" ? `Colaboraste con ${gameState.lastCollab.creatorName}.` : gameState.lastCollab.estado === "rechazada_por_creador" ? `${gameState.lastCollab.creatorName} rechazó tu propuesta.` : `Rechazaste la colab de ${gameState.lastCollab.creatorName}.`}</b><span>${gameState.lastCollab.estado === "aceptada" ? `+${nf(gameState.lastCollab.vistas || 0)} vistas · +${nf(gameState.lastCollab.subs || 0)} subs` : "La relación bajó un poco."}</span></div><a class="btn ghost" href="#collabs">VER DETALLE</a></div>` : ""}
            ...
            ${hizoPretemporada && !p.videoSubidoEsteTrimestre && !pendingEvent && !sponsor && !campaign && !collab ? `<a href="#publish" class="btn primary big pulse publish-main-cta">📹 PUBLICAR VIDEO</a>` : ""}
            ${hizoPretemporada && p.videoSubidoEsteTrimestre && gameState.lastQuarterResult && !pendingEvent && !sponsor && !campaign && !collab ? `<a href="#videoResult" class="btn primary big pulse publish-main-cta">📊 CONTINUAR CIERRE DEL TRIMESTRE</a>` : ""}
