...
import { emojiSvgify, startEmojiSvgObserver } from "./components/emojiSvg.js";
...
function renderScreen(elementId, screenModule) {
    ...
    // Renderizamos primero la nueva pantalla y recién después ocultamos la anterior.
    // Así nunca queda un fondo vacío ni un loader tapando la interfaz durante una
    // navegación normal (minijuegos, "Pasan cosas", sponsors, resultados, etc.).
    try {
        let result;
        ...
        document.querySelectorAll(".screen").forEach(screen => {
            screen.style.display = screen === el ? "block" : "none";
        });
